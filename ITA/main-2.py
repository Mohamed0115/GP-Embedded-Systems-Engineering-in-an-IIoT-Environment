...
from receiver import receive  # Corrected from 'receiver'
...
async def main_async():
    ...
    signal_path = input("Enter signal path (1-6): ") or 1
    ...
    try:
        ...
        while True:  # Runs forever; Ctrl+C to stop
            ...
            if user_input == 'c':
                ...
                # Send custom command
                print(f"Sending custom command: {user_input}")
...
def input_with_timeout(prompt, timeout=5):
    """Synchronous timed input."""
    print(prompt, end='', flush=True)
    loop = asyncio.get_event_loop()
    future = loop.run_in_executor(None, sys.stdin.readline)
    try:
        return loop.run_until_complete(asyncio.wait_for(future, timeout)).strip().lower()
